...
| **02** | Mapping performance | **agreement** with HuBMAP HRA crosswalks — 315 cell types, 22 organs, top-1 56.8 %, top-5 85.7 %. This is *not* accuracy against truth: the crosswalk is itself a lexical label→CL map, so it trusts the label, and `cl_resolve` reproduces it on **312 of 315**. Panel b is the label-independent measure — hand-curated gold across **7 organs, pooled top-1 55.0 % (n=200)** — against the achievable ceiling. Lung, Kidney and Heart (marked ▲) were curated after the method was fixed and were never tuned on: on them top-1 is **59.8 %** and top-5 **94.6 %**, so the method holds up held-out; panel c is the top-1/top-5 gap (median 28 points) |
